// Card renderer — Single Source of Truth for card rendering.
// Used by card-lab-v2.html (CSS holo) and HoloCard.js (DOM overlay card).
// Deterministic: same name → same card, no database needed.
//
// Landscape layout (880x630): left column (identicon + QR) | divider | right column (data).

// ═══════════════════════════════════════════════════════════════
//  CONFIG
// ═══════════════════════════════════════════════════════════════
export const CARD_VERSION = 2;       // Bump when layout/DNA/rendering changes
export const CW = 880, CH = 630;    // Landscape canvas dimensions
const PAD = 20;
...
//  DRAWING HELPERS
// ═══════════════════════════════════════════════════════════════
export function drawCorners(ctx, x, y, w, h, color, size) {
  const L = size || 25, T = 2;
  ctx.fillStyle = color;
...
export function drawIdenticon(ctx, x, y, size, seed, color1, color2) {
...
// Code 128B barcode encoder (better looking than simple random bars)
...
export function drawBarcode(ctx, x, y, w, h, text, color) {
...
export function drawGradientDivider(ctx, x, y, w, color1, color2) {
...
// ═══════════════════════════════════════════════════════════════
//  HOLOS & RARITIES — 4 holo types (aligned with card-lab)
// ═══════════════════════════════════════════════════════════════
export const HOLOS = [
...
// ═══════════════════════════════════════════════════════════════
//  CARD DNA — aligned with card-lab (holo not holoPattern)
// ═══════════════════════════════════════════════════════════════
export class CardDNA {
  constructor(name) {
    this.name = name;
    const h = fnv1a(name);
...
//  options.certId      — override auto-generated certificate ID
//  options.accountType — override default 'INDIVIDUAL' type
// ═══════════════════════════════════════════════════════════════
...
export function renderCard(canvas, name, dna, options) {
  const opts = options || {};
  ...
  if (ctx.measureText(displayName).width > RIGHT_W) {
...
// ═══════════════════════════════════════════════════════════════
//  CARD SERIALIZATION — save/load card state as portable JSON
// ═══════════════════════════════════════════════════════════════

/**
 * Serialize a card to a portable JSON-safe object.
 * Stores everything needed to reproduce the card on any system.
 */
export function serializeCard(name, options = {}) {
  const dna = new CardDNA(name);
  const certId = options.certId || generateCertId(name);
  const accountType = (options.accountType || 'individual').toLowerCase();
  return {
    v: CARD_VERSION,
    name,
    certId,
    ...
    holoType: HOLOS[dna.holo].id,
    rarityName: RARITIES[dna.rarity].name,
    paletteName: PALETTES[dna.palette].name,
  };
}

/**
 * Render a card from a serialized object (or raw name + options).
 * Accepts output of serializeCard() or { name, certId, accountType }.
 */
export function renderFromSerialized(canvas, data) {
  const name = data.name;
  const dna = new CardDNA(name);
  return renderCard(canvas, name, dna, {
    certId: data.certId,
