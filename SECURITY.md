...
- The Cloudflare Worker `dmv-agentcommunity` (`/api/register`, public certificate
  verification at `/api/lookup`, `/api/card`, `/api/og`, `/badge/*`,
  `/c/:id/:name`)
- Supabase Edge Functions (secret-gated registration and `lookup-agent`
  upstreams, badge)
...
- Public certificate verification is only `GET /api/lookup?id=CERT-ID` on the
  ...
  bucket, and returns a minimized six-field response. The Supabase
  `lookup-agent` Edge Function is an internal upstream: it requires the
  ...
  creating a Supabase client. Domain lookup is unsupported.
