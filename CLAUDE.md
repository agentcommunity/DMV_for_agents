...
Cloudflare Workers Static Assets + a Cloudflare Container (Node 20 + `@napi-rs/canvas` Skia) for card rendering. Worker `dmv-agentcommunity` on Taqanu account, container instance type `lite`. The Cloudflare Git integration's automatic build from merged `main` is the single authoritative production deploy path. Production uses `npx wrangler deploy`; non-production uses `npx wrangler versions upload`. The repository guard reads `WORKERS_CI_BRANCH`: `pnpm cf:deploy` deploys only `main`, uploads non-main Workers Builds, and rejects local non-main/detached runs. Use `pnpm cf:preview` for an explicit upload-only local preview. Observe automatic builds; use the local-main deploy fallback only after confirming that an automatic build is neither active nor already started, and never run both paths concurrently. On 2026-08-02, an accidental non-main branch promotion to production made v3 live and established the forward-only v1/v2/v3 production floor. Local dev: `pnpm cf:dev`. Architecture, cache hierarchy, and drift invariants: [CLOUDFLARE.md](CLOUDFLARE.md). Broader migration history + container rename gotcha: `docs/admin/CLOUDFLARE-MIGRATION.md` in the `agentcommunity_page` repo.
...
**CLI source-target data flow (v3 live):** Boot screen (about/terms/charter menu) → step-by-step form (agent name → operator [required] → email → description) → confirmation summary → Y/n gate → POST to `/api/register` worker (with `signup_source: 'cli'` and `machine_fingerprint`) → worker checks shared CF rate limits → worker transactionally claims an exact `REGISTER_FINGERPRINT_LIMITER` slot → worker forwards to Supabase → a well-formed fresh mint commits, only audited pre-INSERT responses or an exact replay release, and every uncertain outcome stays pending → success screen: view card link (permalink to holographic card), share nudge (invite command + card URL), save card (direct PNG download URL), badge markdown snippet, email verification note.
...
**Worker `/api/register` contract** (`worker/index.ts` `handleRegister`): canonical registration entry point for browser, CLI, MCP, and JS API traffic. This branch owns anti-abuse through Turnstile on the browser path, shared Cloudflare rate limiters (`RL_OTP_EMAIL` namespace `4005`, `RL_OTP_IP_EMAIL` namespace `4007`), and an exact live `REGISTER_FINGERPRINT_LIMITER` SQLite Durable Object budget on the CLI/MCP path. One object per SHA-256 fingerprint hash transactionally reserves capacity before upstream. A well-formed fresh `201` commits; only audited, well-formed pre-INSERT `400`/`403`/`409` responses and exact `200 already_recorded` replays release. Every 5xx/546, malformed/unexpected response, body-read failure, timeout/abort, and transport failure stays pending. The 45-second local response deadline does not claim to stop the remote function. Abandoned claims remain pending for 600 seconds (150-second Supabase request-idle timeout + 400-second Edge Function wall clock + 50-second margin), then count for a full 24 hours. CAPTCHA runs before shared counters; `DMV_PROXY_SECRET` presence is checked before any quota. The secret-gated Supabase upstream rejects direct access with 403 `direct_access_deprecated`.
...
**Lookup rollout record and recovery:** the Worker-first deployment, secret-gated Edge deployment, and final smokes completed 2026-07-22. Evidence: issued `REEF-068-BD0Q` returned `200` for `masato`; generated absent `ZZZZ-FFF-FFFD` returned `200 not_found`; `INVALID` returned `400`; calls 1–30 passed, 31 returned `429`, and a next-minute call returned `200` with remaining `29`; health, card, badge, permalink, and validation-only registration checks passed. Cloudflare's runtime requires `redirect: 'manual'`, not `error`; retain manual redirects while treating every 3xx as fail-closed and never following a redirect with the secret. Current production recovery must preserve deployed v1/v2/v3 and all three exports/bindings together. Never publish the secret or document a direct Edge URL as a client API. See `packages/dmv-agent/DEPLOY.md` for recovery steps.
...
**NPM package** (`packages/dmv-agent/`): Published canonically as `@agentcommunity/dmv-agent@0.2.2`; `dmv-agent@0.1.2` is an unscoped compatibility alias in `packages/dmv-agent-alias/`, not a second capability. Source `0.3.0`/`0.1.3` remains unpublished. CLI, MCP server, JS API, Claude Code `/dmv` skill. TypeScript, pnpm for dev, `bunx @agentcommunity/dmv-agent` for users. Only runtime dep: `@modelcontextprotocol/sdk`. Package support is maintained Node LTS (`>=22`, tested on 22/24); root `pnpm-lock.yaml` is the only lock authority. `pnpm verify:packages -- --registry-mode=current` owns clean pack/install/CLI/MCP/current-registry proof, performs no production operation by default, and removes generated package `dist`. Its separate `--production-smoke` is non-registration but consumes live quota and may populate caches. Owner-authorized publishing uses only `.github/workflows/publish-dmv-packages.yml`: unprivileged exact-artifact preparation/state/proof surrounds two main-only protected `npm-production` OIDC publish jobs, canonical before alias, with absent-or-exact resume semantics and cryptographic provenance proof. CLI sends `signup_source: 'cli'`, MCP sends `'mcp'`.
...
- `rate-limit.ts` — Machine fingerprint (SHA-256 of hostname+user+platform) + local lockfile (`~/.dmv-agent/registrations.json`, 3/machine/24h). Advisory client-side check; the live v3 Worker enforces the exact budget via `REGISTER_FINGERPRINT_LIMITER`.
