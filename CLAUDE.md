...
**Worker `/api/lookup`** (`worker/certificate-lookup.ts`): the only public live certificate lookup. Accepts `GET ?id=CERT-ID` only; domain enumeration is removed. It validates the check digit before quota, enforces 30 requests/60s/IP using `RL_CERT_LOOKUP` plus an authoritative `REGISTER_COOLDOWN_KV` bucket, then reads `BADGE_CACHE_KV` (300s for issued, 60s for not found). Public responses are `private, no-store` and contain only `certificate_id`, `status`, `valid_format`, `issued`, `agent_name`, and `certificate_url`. `issued: true` means the registration row exists; it does not mean email verification, name allocation, or DNS delegation completed.

**Edge functions** (`supabase/functions/`): `register-agent` and `lookup-agent` are internal Worker upstreams authenticated by the shared `DMV_PROXY_SECRET`; direct Supabase calls are unsupported and return 403 `direct_access_deprecated`. `lookup-agent` accepts certificate IDs only and returns the Worker only the stored certificate ID and requested name. `badge` remains behind the Worker's public `/badge/*` proxy. All are Deno, deployed to Supabase. Zero secrets in client code.
