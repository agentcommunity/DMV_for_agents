...
**Worker `/api/lookup`** (`worker/certificate-lookup.ts`): implementation-ready but unpublished as of 2026-07-22. After Task 7 records a deployed SHA and live smoke evidence, it will be the only public certificate lookup. It accepts `GET ?id=CERT-ID` only; domain enumeration is removed. It validates the check digit before quota, enforces 30 requests/60s/IP using `RL_CERT_LOOKUP` plus an authoritative `REGISTER_COOLDOWN_KV` bucket, then reads `BADGE_CACHE_KV` (300s for issued, 60s for not found). Public responses are `private, no-store` and contain only `certificate_id`, `status`, `valid_format`, `issued`, `agent_name`, and `certificate_url`. `issued: true` means the registration row exists; it does not mean email verification, name allocation, or DNS delegation completed.

**Edge functions** (`supabase/functions/`): `register-agent` is a live internal Worker upstream authenticated by `DMV_PROXY_SECRET`; direct Supabase calls return 403 `direct_access_deprecated`. The staged, unpublished `lookup-agent` change will apply the same boundary after Task 7. It accepts certificate IDs only and returns the Worker three fields: `certificate_id`, `agent_name`, and `domain`. `badge` remains behind the Worker's public `/badge/*` proxy. All are Deno. Zero secrets in client code.

**Lookup rollout order:** configure the same generated `DMV_PROXY_SECRET` on the Worker and Supabase, verify `RL_CERT_LOOKUP`, `BADGE_CACHE_KV`, and `REGISTER_COOLDOWN_KV`, deploy the Worker first, then deploy `lookup-agent --no-verify-jwt`. Never publish the secret or document a direct Edge URL as a client API.
