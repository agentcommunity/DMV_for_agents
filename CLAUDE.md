...
**Lookup rollout order:** configure the same generated `DMV_PROXY_SECRET` on the Worker and Supabase, verify `RL_CERT_LOOKUP`, `CERT_LOOKUP_LIMITER`, and `BADGE_CACHE_KV`, deploy the Worker first, then deploy `lookup-agent --no-verify-jwt`. Never publish the secret or document a direct Edge URL as a client API.
