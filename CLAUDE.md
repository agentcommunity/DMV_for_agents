...
**Worker `/api/lookup`** (`worker/certificate-lookup.ts`): implementation-ready but unpublished as of 2026-07-22. After Task 8 records a deployed SHA and live smoke evidence, it will be the only public certificate lookup. It accepts `GET ?id=CERT-ID` only; domain enumeration is removed. It validates the check digit before quota, applies coarse/eventually consistent `RL_CERT_LOOKUP` at 60/60, then uses one `CERT_LOOKUP_LIMITER` SQLite Durable Object per hashed IP for exact transactional 30/60 accounting and headers. Pre-DO responses omit guessed exact remaining/reset values. Durable Object failure fails closed before reading `BADGE_CACHE_KV` (300s issued, 60s typed not found). Public responses are `private, no-store` and contain only `certificate_id`, `status`, `valid_format`, `issued`, `agent_name`, and `certificate_url`. `issued: true` means the registration row exists; it does not mean email verification, name allocation, or DNS delegation completed.

**Edge functions** (`supabase/functions/`): `register-agent` is a live internal Worker upstream authenticated by `DMV_PROXY_SECRET`; direct Supabase calls return 403 `direct_access_deprecated`. The staged, unpublished `lookup-agent` change will apply the same boundary after Task 8. It accepts certificate IDs only and returns exactly a typed HTTP 200 `issued` or `not_found` envelope; every other Worker-observed response is unavailable and uncached. `badge` remains behind the Worker's public `/badge/*` proxy. All are Deno. Zero secrets in client code.

**Lookup rollout order:** use a Docker-capable gate, configure the same generated `DMV_PROXY_SECRET` on the Worker and Supabase, verify account namespace 1002 plus `RL_CERT_LOOKUP`, `CERT_LOOKUP_LIMITER`, `BADGE_CACHE_KV`, and v1/v2 migrations, then merge `main` for the authoritative Cloudflare Git deploy. Smoke the expected brief Worker-first 503 compatibility interval before deploying only `lookup-agent --project-ref tcymqfwwphacnosnnzxl --no-verify-jwt`. After evidence, the live-status docs commit causes a second automatic Worker deployment; observe that exact commit, capture its final SHA/version, and repeat the minimum lookup, health, card, badge, and validation-registration smokes before handoff. Never roll back to pre-v2; roll forward while preserving migrations/export/binding. Never publish the secret or document a direct Edge URL as a client API. See `packages/dmv-agent/DEPLOY.md` for the canonical commands, evidence, and recovery steps.
