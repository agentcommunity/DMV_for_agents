...
**Package release contract:** npm currently publishes canonical `@agentcommunity/dmv-agent@0.2.2` and compatibility alias `dmv-agent@0.1.2`; source `0.3.0`/`0.1.3` is not published. Node 22 and 24 are the maintained test matrix and package support is `>=22`. Root `pnpm-lock.yaml` is the sole workspace lock authority. Run `pnpm verify:packages -- --registry-mode=current`; it is pure with respect to production operations and must leave no package `dist`. Run `--production-smoke` separately and deliberately: it performs no registration or business-data write, but it consumes lookup/rate-limit quota and may populate caches. Publication is owner-authorized only through `.github/workflows/publish-dmv-packages.yml`. Unprivileged jobs build, execute, and prove exact artifacts; only the two protected `npm-production` jobs receive OIDC, download those artifacts, and run exact `npm publish <tgz> --access public --provenance` commands. The absent-or-exact sequence is canonical state/publish/proof, then alias state/publish/final proof; a mismatch requires a version bump. npm registry signatures and cryptographically verified provenance attestations are distinct requirements. Never add a release token, publish manually, or use a live registration as a smoke test.
